...
if (isset($_POST["Import"])) {
    ...
    if ((($_FILES["file"]["type"] == "image/gif")
        ...
        if ($_FILES["file"]["size"] > 0) {
            ...
            $sql2 = mysqli_query($link, "INSERT INTO `calling_lead_title` (`SESSID`, `DATED`,`LEAD_CATEGORY`,`LEAD_DATE`,STATUS)
            values('$SESSID', NOW(),'$LEAD_CATEGORY','$LEAD_DATE','1')");
            ...
            while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
                ...
                if ($i > 0) {
                    ...
                    if ($SelectSID['ID'] != '') {
                        ...
                        $selectquerys = mysqli_num_rows(mysqli_query($link, "SELECT * FROM `calling_lead` WHERE RMS_ID ='". $emapData[0] ."'"));
                        if ($selectquerys == '0') {
                            $sql = mysqli_query($link, "INSERT INTO `calling_lead` (`LEADTID`, `DATED`, `RMS_ID`, `PHONE`, `EMAIL`, `PREFFERED_COUNTRY`, `REGISTER_DATE`,`SENDING_COUNTRY`,`TRANSACTION_COUNT`,`LAST_TRANSACTION_DATE`)
                            values('$LEADTID',NOW(),'$emapData[0]', '$emapData[1]', '$emapData[2]', '$emapData[3]', '$emapData[4]', '$emapData[5]', '$emapData[6]', '$emapData[7]')");
                        }
                    }
                }
                $i++;
