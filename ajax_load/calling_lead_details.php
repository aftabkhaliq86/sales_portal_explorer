...
<table id="leads" class="col-lg-12 table-striped table-condensed cf tbl table-responsive">
    ...
    <tbody>
        ...
        if (isset($_GET['AGID'])) {
            ...
            $STATUS_TITLE_STATUS = '';
            $DATED_sclc = '';
            $result = mysqli_query($link, "SELECT `cl`.*,`clg`.`PERSON_NAME` FROM `calling_lead` AS `cl` INNER JOIN `calling_lead_agents` AS `clg` ON `cl`.`USERID`=`clg`.`ID` WHERE `cl`.`LEADTID`='$ID' AND `cl`.`USERID`='$AGID' ORDER BY `cl`.`DATED`");
            $counters = '0';
