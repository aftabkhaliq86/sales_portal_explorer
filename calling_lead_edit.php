...
if (isset($_POST['callingagentedit'])) {
    ...
    $sql = "UPDATE `calling_lead_title` AS clt,`calling_lead` AS cl SET `clt`.`LEAD_CATEGORY`='$LEAD_CATEGORY',`clt`.`LEAD_DATE`='$LEAD_DATE',`clt`.`STATUS`='$STATUS',`cl`.`INACTIVE_LEAD_TITLE`='$STATUS' WHERE `clt`.`ID`= `cl`.`LEADTID` AND `clt`.`ID` = $RID AND `cl`.`LEADTID` = $RID";
